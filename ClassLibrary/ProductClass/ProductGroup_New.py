...
class ProductGroup(Object):
    def __init__(self):
        ...
        self.propertyOption = None
        self.productService = None
        self.ip = None
        self.product = None
        self.brand = None
        self.saleCategory = None
        self.storeCategory = None
        self.mainImage = None
        self.imageList = None
        self.collectionUser = None
        self.comment = None
        self.briefDescription = None
        self.detailDescription = None
        self.shop = None
        self.saleCount = None
        self.price = None
        self.spec = None
        self.freightModel = None
        self.dispatchPlace = None
    ...
    def get_attribute_productService(self):
        if self.instance:
            productService = Base.get_relation_data(self.instance.get(attribute_objectId), self.__class__.__name__, attribute_productService)
            productServiceList = []
            if productService:
    ...
    def get_attribute_product(self):
        if self.instance:
            productList = Base.get_relation_data(self.instance.get(attribute_objectId), self.__class__.__name__, attribute_product)
            if productList:
                product = []
    ...
    def get_attribute_comment(self, attribute, value, skip=None, limit=None):
        if self.instance and attribute:
            tmpList = Base.get_relation_data_and_attribute(self.instance.get(attribute_objectId), self.__class__.__name__, attribute_comment,
                                                           attribute, value,
                                                           skip, limit)
            if tmpList:
                returnList = []
    ...
    def get_attribute_comment_count(self, attribute, value):
        if self.instance and attribute:
            count = Base.count_relation_data_and_attribute(self.instance.get(attribute_objectId), self.__class__.__name__, attribute_comment,
                                                           attribute, value)
            return count
        return None
    ...
    def get_attribute_detailDescription(self):
        if self.instance:
            imageList = Base.get_relation_data(self.instance.get(attribute_objectId), self.__class__.__name__, attribute_detailDescription)
            if imageList:
                image = []
                for foo in imageList:
    ...
    def create_ProductGroup(self, data, shop):
        ...
        if shop.set_attribute_productGroup(self.instance):
            # 将店铺信息写入到productGroup中
            self.set_attribute_shop(shop.get_instance())

            storeCategory = StoreCategory(Class_Name_StoreCategoryThird)
            store = storeCategory.get_Object(data[attribute_storeCategory])

            if not self.set_attribute_storeCategory(store) or not storeCategory.set_attribute_productGroup(self.instance):
            ...
            if data[attribute_saleCategory]:
                for foo in data[attribute_saleCategory]:
                    ...
                    # 将商品组写入到对应的销售关系中
                    if not self.set_attribute_saleCategory(sale) or not saleCategory.set_attribute_productGroup(self.instance):
            ...
            Base.sys_log('product can not save in shop')
        return None
    ...
    def update_ProductGroup(self, data):
        if data and self.instance:
            ...
            if data[attribute_spec] and self.get_attribute_spec() != data[attribute_spec]:
                self.set_attribute_spec(data[attribute_spec])
            if data[self.propertyOption]:
                propertyOption = data[self.propertyOption].replace("'", "\"")
            ...
            if data[attribute_saleCategory]:
                ...
                oldSaleCategory = Base.get_relation_data(self.get_attribute_objectId(), self.__class__.__name__, attribute_saleCategory)
                if oldSaleCategory:
            ...
            oldProductService = Base.get_relation_data(productGroup.get(attribute_objectId), self.__class__.__name__, attribute_productService)
